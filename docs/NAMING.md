...
| **Plugin slug (specific plugin)** | `summer` | Plugin manifest `name` field across plugin-capable harnesses (Claude / Cursor / Codex / Factory / Gemini), slash command namespace, CLI binary, OpenCode plugin name. |
...
| Public agent-layer repo | `SummerEngine/summer-engine-agent` |
...
| GitHub Copilot CLI setup target | `github-copilot` |
| GitHub Copilot in VS Code setup target | `vscode-copilot` |
...
This repo is not just a CLI. It is the public **Summer agent layer**: CLI, MCP bridge, skills, hooks, setup targets, and plugin manifests.

The public repo is `SummerEngine/summer-engine-agent`. Keep these surrounding names stable:

- Keep the npm package as `summer-engine`. Users are trying to install Summer Engine, and `npx -y summer-engine@latest ...` reads correctly.
- Keep the CLI binary as `summer`. It is short, memorable, and works as the command namespace.
- Keep `SummerEngine/summer-engine-agent` as the public agent-layer repo.
- Reserve `SummerEngine/summer-engine` for a future public engine-source repo if the desktop engine is ever opened. Do not burn that name on the agent layer.
...
| Identifying this repo in a sentence | "Summer is the open-source agent layer that..." | When distinguishing the agent layer from the engine app. |
...
- [ ] All plugin manifests have `name: "summer"` (Claude, Cursor, Codex, Factory) or extension equivalent (Gemini).
