...
`align-distribute-3d`, `navigation-probe`, `starcast`, and the editor UI control
four `ui-actions`, `ui-tree`, `ui-activate`, `ui-screenshot`
(`grep -l 'status: preview' library/tools/*/resource.yaml` is the live list). Calling one returns a
