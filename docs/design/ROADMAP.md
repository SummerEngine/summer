...
- Marcus's spatial work: `summer_starcast` (engine PR #147, `codex/starcast-spatial-probe`) is superseded by his own six-tool suite (`codex/world-tool-balanced-suite-ready`, relabels starcast "legacy") — the suite is ported (preview until its engine ops merge). Decision: do NOT port starcast; ask Marcus to open the world-tool engine half as a PR like #155/#156. His web PR #290 (embedded-agent `spatial-placement` skill) stays in the web repo.
...
- `SummerEngine/summer-gamedev-knowledge` (private) is fed by the `gamedev-knowledge` Slack bot — evaluate as an intake source for the content factory (§3.5): links → summaries → candidate entries → CI gate → review.
