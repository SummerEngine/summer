...
### Added 2026-09-03 (Navigation — `summer open` / `summer_open`) ✅ toolkit side
- **Shipped on `v3-foundation`:** `tool/open` (MCP `summer_open`, CLI `summer open <target>`, `summer tool open` — one behavior in `src/core/capabilities/navigation/`), `reference/product-map` (every web page and editor surface with intent phrases, URL/op, required context, implemented vs planned), `skill/navigate-summer` (open a surface for the user vs act via API). Web targets verified against the web repo's route files; editor targets against `op_registry.json`. Login deep-links through `/login?returnUrl=`. Research: `NAVIGATION-RESEARCH.md`; design: `NAVIGATION-DESIGN.md`.
- **Engine ops needed to flip the 11 `planned` editor targets** (one dispatch branch + handler each; the C++ exists behind the chat webview bridge): `SetMainScreen{screen}` (`EditorInterface::set_main_screen_editor`), `FocusChat{path?}` (`ChatDock::open_chat_path`), `ChatInjectMessage`, `OpenProjectSettings{tab?}` (`ProjectSettingsEditor::popup_project_settings`, needs an EditorNode accessor), `OpenEditorSettings`, `ShowBottomPanel{panel}` (`EditorBottomPanel::make_item_visible` + name resolver), `FocusEditorWindow` (`DisplayServer::window_move_to_foreground`), `OpenScript{path,line,col,grab_focus}` (today `OpenResource` hard-codes `grab_focus=false`), extend `_se_resolve_dock` with chat/changes/import/signals/groups. Owner: engine. When each lands: flip the row's `status` in `targets.ts` + `product-map.md`, add the op to `CLI_KNOWN_OP_NEEDS`.
- **v2, not scheduled:** a `summer://` navigation scheme. The engine already registers `summerengine://` (macOS, auth-only) and spawns a second editor when one is running; a navigation scheme must forward to the running instance over the local API first (VS Code pattern), and Windows registration is a TODO stub. Until then the CLI/MCP is the only entry point, by design.
- **Web-repo PR list (proposals, nothing edited):** (1) publish a machine-readable route/intent catalog (`/agent-routes.json` or a `routes` key in `agent-catalog.json`) generated from the same module as the internal `studio/lib/ask-summer-registry.ts`, so `targets.ts` can be checked in CI instead of hand-verified; (2) add a "Navigating summerengine.com" section to `llms.txt` and `<link rel="alternate" type="application/json">` on `/for-ai`; (3) validate `next` on `/auth/callback` through `resolveReturnPath` (today string-concatenated: `?next=//host` is a protocol-relative open redirect); (4) make `/dashboard/settings` actually redirect to `/studio?tab=settings` (declared but not enforced); (5) derive `toolsNumber`/`lastModified` in `agent-catalog.ts` from `counts.json` / a build stamp instead of the hardcoded `44` / `2026-07-30`; (6) v2: a `/open?to=<target>` router resolving the same catalog server-side (Cloudflare `?to=` pattern) so links in chat/docs survive route moves.
- **Not verified yet:** a live-engine `summer open scene` / `node` (no editor with the API up during the build) — run TESTING.md §d with `summer open inspector` once an editor is open.
