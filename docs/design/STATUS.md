...
| Library: 188 resources validate (70 tools / 91 skills / 19 templates / 8 references), capability lint clean (documented exceptions only), `remote` explicit on every tool, authority fields audited against implementations. 11 skills are `preview` (7 = source-cited intake from Marcus's gamedev-knowledge pipeline, 4 depend on unmerged engine ops); `summer setup` installs them labelled by default, `--stable-only` skips | `validate:library` + resource.yaml audit |
...
13 tools gated on unmerged engine ops degrade with `engine_lacks_op` (override: `SUMMER_CAPABILITY_PREFLIGHT=off`):
...
- Live-engine verification of the 13 preview tools: impossible until engine PRs #155/#156 merge (the engine-free and 57 engine tools ARE live-verified).
