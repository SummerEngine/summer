...
CUR_PROJECT=""
cleanup_engine() {
  [[ -n "$COPIER_PID" ]] && { kill "$COPIER_PID" 2>/dev/null || true; wait "$COPIER_PID" 2>/dev/null || true; COPIER_PID=""; }
  ...
  if [[ -n "$EDITOR_PID" ]]; then
    # our own stale registry entry (engine normally removes it on exit)
    for f in "$REAL_HOME"/.summer/instances/*.json; do
      [[ -f "$f" ]] || continue
...
run_smoke() { # $1 project  $2 log  -> rc
  ...
  ( cd "$1" && bash tests/autopilot/run.sh ) >"$2" 2>&1 &
  SMOKE_PID=$!
  wait "$SMOKE_PID" || rc=$?
  SMOKE_PID=""
  return $rc
...
run_task() {
  ...
  local t0=$SECONDS
  "$SUMMER_BIN" --editor --path "$project" --summer-offscreen --summer-no-publish --disable-crash-handler >"$out/editor.log" 2>&1 &
  EDITOR_PID=$!
  log "editor pid $EDITOR_PID launched (offscreen)"
  local inst
  if ! inst="$("$NODE" "$LIB" wait-instance "$project" "$EDITOR_PID" "$REAL_HOME" "$FAKE_HOME" 240 2>>"$out/run.err")"; then
  ...
  # keep the fake HOME's copy fresh (heartbeatAt goes stale after 180 s)
  ( while kill -0 "$EDITOR_PID" 2>/dev/null; do cp "$inst_file" "$FAKE_HOME/.summer/instances/" 2>/dev/null || true; sleep 5; done ) &
  COPIER_PID=$!
