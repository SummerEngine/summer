---
name: skill-improve
description: Iterate on an existing Summer skill using a parallel-eval harness — run the skill against a behavioral spec with and without proposed changes, compare outputs, ship the version that wins. Use to upgrade a skill that's underperforming. Trigger on "improve skill", "iterate on skill", "make this skill better".
license: MIT
...
- **Test cases to focus on.** Default: all `## Case` blocks in `_tests/specs/<name>.md`.
...
Save outputs to `_tests/runs/<skill-name>/baseline/case-<N>/`.
...
Draft a revised SKILL.md. May I write it to `_tests/runs/<skill-name>/proposed/SKILL.md`?

### 4. Run the proposed version

Repeat step 2 with the proposed SKILL.md. Save to `_tests/runs/<skill-name>/proposed/case-<N>/`.
...
> Proposed version wins 2 cases, ties 1, loses 0. May I overwrite `skills/<category>/<name>/SKILL.md` with the proposed version and update `last-tested:` in `catalog.yaml`?

### 6. Ship

On user yes:
- Overwrite `SKILL.md`.
- Update `last-tested: <today>` in `catalog.yaml`.
