...
For a single skill, read both `skills/<category>/<name>/SKILL.md` and `_tests/specs/<name>.md`. For each `## Case` in the spec:
...
Read `catalog.yaml`. For each skill with `status: HAVE`:

1. Run `static` mode on the skill.
2. Note the skill's `last-tested:` date.
3. Flag skills where `last-tested:` is missing or older than 30 days.

Output: a table with PASS / FAIL counts, last-tested ages, and a punch list of skills needing attention.
...
- `_shared/template-registry.md`
- `_tests/runner.md`
- `_tests/specs/` — per-skill behavioral specs
