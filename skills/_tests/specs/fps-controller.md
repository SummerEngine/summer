...
- [ ] Skill never calls `summer_set_resource_property` on an inline `sub_resource` (the silent-fail trap from `_shared/mcp-tools-reference.md`).
- [ ] Final step is `summer_save_scene` followed by `summer_get_script_errors`.
- [ ] Script uses typed GDScript per `_shared/gd-style.md` (`@export`, `:=`, `class_name`).
...
This spec runs via `/skill-test fps-controller spec` (see `_meta/skill-test/SKILL.md`).
