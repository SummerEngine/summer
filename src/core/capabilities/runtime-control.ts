...
export const playGameExtensionSchema = {
  ...
  seed: z
    ...
    .int()
    .optional()
    .describe("Global RNG seed passed as --summer-seed (default 20260725 when deterministic). Pins randi/randf/shuffle/pick_random only — see the result's determinism.seed_scope for what it does NOT pin."),
  fixed_fps: z
    .number()
    .int()
    .optional()
    .describe("Run the game at a fixed frame rate (--fixed-fps N) so at_ms input timing maps exactly and GameStep frames are stable."),
  speed: z
    .number()
    .optional()
